...
import { createElement, appendChild, setPropsForDom, createText } from "react-dom-bindings/src/client/ReactDOMHostConfig";
...
function appendChildren(parent, workInProgress) {
    ...
    while(fiber !== null) {
        if([HostComponent, HostText].includes(fiber.tag)) { // 节点类型为标签或文本，直接挂载
            appendChild(parent, fiber.stateNode)
            fiber = fiber.sibling
        } else if(fiber.child !== null) { // 节点类型为函数组件、类组件等，没法直接挂载，那么拿它的子节点
            fiber = fiber.child
            continue
        }
        console.log(parent);
